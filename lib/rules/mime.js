...
module.exports = async function validateMime({ value, args }) {
  ...
  if (file.mime) {
    ...
    try {
      mtype = (await fileType.fileTypeFromBuffer(file)).mime;
    } catch (e) {
    ...
    try {
      mtype = (await fileType.fileTypeFromBuffer(file.buffer)).mime;
    } catch (e) {
      /* istanbul ignore next */
      console.error(e);
    }
    ...
    try {
      ...
      mtype = (await fileType.fileTypeFromBuffer(buffer)).mime;
    } catch (e) {
      /* istanbul ignore next */
      console.error(e);
    }
